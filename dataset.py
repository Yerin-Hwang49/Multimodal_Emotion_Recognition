...
class KEMDy20(Dataset):
    def __init__(self, path, text_feat_path, wav_feat_path):
        ...
        with open(text_feat_path, 'r') as f:
            KoBERT_output = json.load(f)
        with open(wav_feat_path, 'r') as f:
            wav_output = json.load(f)
    ...
    def collate_fn(self, batch):
        class_to_label = {'neutral':0,'angry':1,'disqust':2,'fear':3,'happy':4,'sad':5,'surprise':6}
        ...
        is_neutral = [1 if data['Emotion']=='neutral' else 0 for data in batch]
